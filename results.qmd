...
## 1.- Preliminary Analysis - Aggregate Numbers
...
The bar chart above identifies 10 most common causes of death in the United States aggregated from year 1999 to 2017. It's is visible that heart disease and cancer account for a significant bulk of all deaths. We will focus our analysis on these.
...
### 1.2. Understanding overall trends
...
-   The Crude Death Rate, which is simply the number of deaths divided by the total population reflects this fact going down to 665 from 680. This means that the death rate went down 2.2%.
...
### 1.3. Understanding the underlying causes of death
...
ggplot(cdc_usa_wider, aes(x = Year, y = `Cumulative Change`, color = Metric)) +
  geom_line(size = 1.3) +
  labs(title = "Cumulative Change - Age Adj. Deaht Rate", x = "Year", y = "Cumulative Percentual Change", color = "Metric") +
  theme_minimal() +
...
ggplot(df_rank1, aes(x = PercentChange, y = reorder(State, PercentChange))) +
  ...
  labs(title = "Alzheimer's Disiease %Chg. in Age Adj. Death Rate", x = "Percentage Change 1999 to 2020", y = 'State') +
  theme_bw() +
...
ggplot(df_rank2, aes(x = PercentChange, y = reorder(State, PercentChange))) +
  ...
  labs(title = "Unintentional Injuries %Chg. in Age Adj. Death Rate", x = "Percentage Change 1999 to 2020", y = 'State') +
  theme_bw() +
  theme(panel.grid.major.x = element_blank(),
    panel.grid.minor.x = element_blank(),
    panel.grid.major.y = element_line(colour = "grey60", linetype = "dashed"))+
...
ggplot(data = df_map, aes(x = long, y = lat, group = group, fill = Top10Percent)) +
    ...
    scale_fill_manual(values = c("Top 12%" = "#843722", "Below Top 12%" = "#ffe0bd"), 
    ...
    coord_fixed(1.4) +
    labs(title = "Top 12% of States by Percent Change in Death Rates due to Alzheimer's")
...
ggplot(cdc_usa_race, aes(x = Year, y = `Cumulative Change`, color = Alzheimer)) +
  geom_line(size = 1.3) +
  labs(title = "Cumulative Percentual Change ('99-'20) - Alzheimer - Age Adjusted Death Rate", x = "Year",
       y = "Cumulative Percentual Change", color = "Race") +
  theme_minimal() +
  theme(legend.position = "bottom") +
  scale_y_continuous(labels = scales::percent_format(), breaks = scales::pretty_breaks(n = 8))+
  scale_x_continuous(breaks = scales::pretty_breaks(n = 10))
...
#We calculate the 88th percentile (top 12% threshold)
cdc_usa_race_pop_threshold <- quantile(cdc_usa_race_pop$Black, probs = 0.75, na.rm = TRUE)
...
ggplot(data = df_map, aes(x = long, y = lat, group = group, fill = TopPercent)) +
    geom_polygon(color = "black") +
    scale_fill_manual(values = c("Top 25%" = "#843722", "Below Top 25%" = "#ffe0bd"), 
    ...
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          ...
          axis.title.x = element_blank(), axis.title.y = element_blank()) +
    coord_fixed(1.4) +
    labs(title = "Top 25% of States with more Black People")
...
### 1.6. Analyzing Unintentional Injuries
