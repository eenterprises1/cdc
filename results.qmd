...
df_opiod_state_arranged %>% filter(Year < 2017) %>% group_by(State) %>% summarize(Avg = mean(OpiodDispenseRatePer100)) %>% arrange(desc(Avg)) %>% filter(Avg > 115)
...
```{r}
df_opiod_state_arranged_top5_states = df_opiod_state_arranged %>% filter(State == "West Virginia" | State == "Alabama" | State == "Tennessee" | State == "Kentucky" | State == "Arkansas")
...
```{r}
cude_rate_states = cdc_dataset %>% filter(State != "United States") %>% filter(Cause == "Unintentional injuries")
cude_rate_states %>% group_by(State) %>% summarize(Avg = mean(Age_Adj_Rate)) %>% arrange(desc(Avg))
```
