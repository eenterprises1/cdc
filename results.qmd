...
ggplot(df_all_3_99_to_20, aes(x = Cause, y = PercentChange, fill = Cause)) +
  ...
  scale_y_continuous(labels = percent_format()) + labs(y = "Percent Change")  
...
# Base dotchart
dotchart(w_reverse$Avg, labels = w_reverse$State, gcolor = "green",
         cex = 0.9, xlab = "Age Adjusted Death Rate", 
         main = "Top 8 Highest States w/ Avg Age-Adj Death Rate for Unintentional '06-'20")
...
ggplot(merged_df, aes(x = log(densityMi), y = Avg)) + geom_point() + geom_smooth(method = "lm") + 
  ...
  labs(
    ...
    caption = "Correlation coefficient R = -0.860"
  ) +
  theme_minimal()
```
```{r}
